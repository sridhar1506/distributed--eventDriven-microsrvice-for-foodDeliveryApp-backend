# Food Booking Platform Architecture (Backend)

This document defines the system architecture, operational flow, and technical specifications for the application's microservices ecosystem.

## 1. System Architecture

The application is built on a distributed microservices architecture, utilizing Spring Cloud for routing and discovery, and Apache Kafka for event-driven asynchronous communication.

```mermaid
graph TD
    Client[External Client/API Tester] -->|HTTP / REST| API_Gateway(API Gateway)
    
    subgraph Spring Cloud Infrastructure
        Service_Registry(Eureka Service Registry)
        Kafka[Apache Kafka Message Broker]
    end
    
    subgraph Microservices Backend
        API_Gateway -->|Routes| Auth_Service(Auth Service)
        API_Gateway -->|Routes| Restaurant_Service(Restaurant Service)
        API_Gateway -->|Routes| Order_Service(Order Service)
        API_Gateway -->|Routes| Payment_Service(Payment Service)
    end
    
    Service_Registry -.->|Registers/Discovers| API_Gateway
    Service_Registry -.->|Registers/Discovers| Auth_Service
    Service_Registry -.->|Registers/Discovers| Restaurant_Service
    Service_Registry -.->|Registers/Discovers| Order_Service
    Service_Registry -.->|Registers/Discovers| Payment_Service
    

    Order_Service -->|Publishes/Consumes Events| Kafka
    Payment_Service -->|Publishes/Consumes Events| Kafka
    Restaurant_Service -->|Publishes/Consumes Events| Kafka
    
    Auth_Service --> DB1[(Auth DB)]
    Restaurant_Service --> DB4[(Restaurant DB)]
    Order_Service --> DB2[(Order DB)]
    Payment_Service --> DB3[(Payment DB)]
    
```

### Backend Specifications
*   **Language**: Java 25
*   **Framework**: Spring Boot 4.0.5
*   **Cloud Infrastructure**: Spring Cloud 2025.1.1 (Gateway, OpenFeign, Netflix Eureka)
*   **Resilience**: Resilience4j (Circuit Breakers, Retries)
*   **Message Broker**: Apache Kafka (Managed via `spring-boot-starter-kafka`)
*   **Security**: Spring Security, JWT (jjwt 0.11.5), Internal API Key Validation
*   **ORM**: Spring Data JPA

### Monitoring & Observability Tools (PLG Stack)
*   **Metrics**: Prometheus (Exposing metrics via OTLP)
*   **Logs**: Grafana, Loki (Log aggregation via OTLP)
*   **Dashboards**: Grafana (Unified visualization)
*   **Distributed Tracing**: Micrometer Tracing & Tempo (OTLP Traces)

### Database Specifications
*   **RDBMS**: MySQL (Local Installation using mysql-connector-j)

## 2. Project Structure

```text
/Food Delivery Application
├── .gitignore              # IDE and system build settings
├── docker-compose.yml      # Infrastructure Orchestration (Variables for all ports)
├── prometheus.yml          # OTLP Metrics Configuration
├── tempo.yaml              # OTLP Tracing Configuration
├── api-gateway/            # Secured Gateway with JWT & Internal Keys
├── auth-service/           # Identity & JWT Management
├── order-service/          # Order Lifecycle & Kafka Events
├── payment-service/        # Ledger & Payment Integration
├── restaurant-service/     # Menu & Stock Management
└── service-registry/       # Eureka Discovery Server
```

## 3. Interservice Communication Map

The backend utilizes **Spring Cloud OpenFeign** for synchronous operations and **Apache Kafka** for asynchronous domain events. All internal requests are secured via an `X-Internal-Key` handshake injected by the Gateway.

```mermaid
sequenceDiagram
    participant Client as API Client
    participant Gateway as API Gateway
    participant Auth as Auth Service
    participant Order as Order Service
    participant Payment as Payment Service
    participant Kafka as Apache Kafka
    participant Restaurant as Restaurant Service

    Client->>Gateway: POST /api/orders
    Note over Gateway: Injects X-Internal-Key
    Gateway->>Order: Route Request /api/orders
    Order->>Auth: Internal Check
    Auth-->>Order: Token Validated
    
    Order->>Kafka: Publish "OrderCreated" Event
    Order-->>Client: HTTP 202 Accepted (Order Pending)
    
    Kafka-->>Payment: Consume "OrderCreated" Event
    Payment->>Payment: Process Payment/Ledger
    
    Kafka-->>Restaurant: Consume "OrderCreated" Event
    Restaurant->>Restaurant: Allocate Stock
    Restaurant->>Kafka: Publish "StockAllocated" Event
    Kafka-->>Order: Consume "StockAllocated" Event
    Order->>Order: Update Status to CONFIRMED
```

## 4. Operational Flow

All data goes through API Gateway which checks Service Registery for the address of respective microservices.
1.  **Authentication**: All requests require a valid JWT generated by the `auth-service`.
2.  **Edge Verification**: The API Gateway intercepts the request, validates the JWT, and attaches a secure `X-Internal-Key` header.
3.  **Service Trust**: Downstream services (Restaurant, Order, Payment etc.) only accept requests that contain the correct `X-Internal-Key`, preventing direct access bypass.
4.  **Observability**: All services push telemetry (logs, metrics, and traces) via **OTLP** to the centralized monitoring stack.

## 5. Startup Order

Everything techs and tools used here are configured to run locally 
1.  **Infrastructure Core**: Kafka, MySQL, Loki, Prometheus, Tempo, Grafana.
2.  **Service Registry (`service-registry`)**.
3.  **API Gateway (`api-gateway`)**.
4.  **Auth Service (`auth-service`)**.
5.  **Core Domain Services (`restaurant-service`, `order-service`, `payment-service`)**.
